...
void LBM_performStreamCollide( LBM_Grid srcGrid, LBM_Grid dstGrid, int z1, int z2) {
  start_slice();
	SWEEP_VAR
...
#endif
#endif
	SWEEP_START( 0, 0, z1, 0, 0, z2)
		if( TEST_FLAG_SWEEP( srcGrid, OBSTACLE )) {
			DST_C ( dstGrid ) = SRC_C ( srcGrid ) + 0.0f;
			DST_S ( dstGrid ) = SRC_N ( srcGrid ) + 0.0f;
			DST_N ( dstGrid ) = SRC_S ( srcGrid ) + 0.0f;
			DST_W ( dstGrid ) = SRC_E ( srcGrid ) + 0.0f;
			DST_E ( dstGrid ) = SRC_W ( srcGrid ) + 0.0f;
			DST_B ( dstGrid ) = SRC_T ( srcGrid ) + 0.0f;
			DST_T ( dstGrid ) = SRC_B ( srcGrid ) + 0.0f;
			DST_SW( dstGrid ) = SRC_NE( srcGrid ) + 0.0f;
			DST_SE( dstGrid ) = SRC_NW( srcGrid ) + 0.0f;
			DST_NW( dstGrid ) = SRC_SE( srcGrid ) + 0.0f;
			DST_NE( dstGrid ) = SRC_SW( srcGrid ) + 0.0f;
			DST_SB( dstGrid ) = SRC_NT( srcGrid ) + 0.0f;
			DST_ST( dstGrid ) = SRC_NB( srcGrid ) + 0.0f;
			DST_NB( dstGrid ) = SRC_ST( srcGrid ) + 0.0f;
			DST_NT( dstGrid ) = SRC_SB( srcGrid ) + 0.0f;
			DST_WB( dstGrid ) = SRC_ET( srcGrid ) + 0.0f;
			DST_WT( dstGrid ) = SRC_EB( srcGrid ) + 0.0f;
			DST_EB( dstGrid ) = SRC_WT( srcGrid ) + 0.0f;
			DST_ET( dstGrid ) = SRC_WB( srcGrid ) + 0.0f;
			continue;
		}
...
void LBM_showGridStatistics( LBM_Grid grid, int z1, int z2 ) {
  start_slice();
