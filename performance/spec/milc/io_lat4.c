...
/* Checksums
   ...
   files appear as a series of 32-bit doubleing point numbers.  We treat
...
/* tolerance for doubleing point checks */
...
#define MAX_TOKENS 512
  ...
  */
  ...
  values = (char **) calloc(MAX_TOKENS, sizeof(char *));
  hdr = (QCDheader *) calloc(1, sizeof(QCDheader));
  (*hdr).token = tokens;
  (*hdr).value = values;
  ...
  while (1) {
    ...
    *q = '\0';
    ...
    p = (char *)calloc((len+1)/8+1, (size_t) 8);
    (*hdr).token[n] = p;
    strcpy(p,line);
...
gauge_file *setup_input_gauge_file(char *filename)
{
  ...
  gf = (gauge_file *)calloc(1, sizeof(gauge_file));
  if(gf == NULL)
    {
  ...
  gh = (gauge_header *)calloc(1, sizeof(gauge_header));
  if(gh == NULL)
    {
  ...
  for(rcv_rank=0; rcv_rank<volume; rcv_rank++)
	...
	for (mu=0;mu<4;mu++) {
	  ...
	  **/
	    ...
	    tmpsu3[mu].e[a][b].real = U[mu][2*(3*a+b)] + 0.0f;
     /*	    printf("real: p=%d, mu=%d, e=%f\n", p,mu,U[mu][2*(3*a+b)]); */
	    tmpsu3[mu].e[a][b].imag = U[mu][2*(3*a+b)+1] + 0.0f;
...
gauge_file *save_serial_archive(char *filename) {
  ...
  FORALLSITES(i,s) {
	...
	tmpflt = s->link[mu].e[a][b].real + 0.0f;
	p32 = (u_int32type *) &tmpflt;
	chksum += *p32;
	tmpflt = s->link[mu].e[a][b].imag + 0.0f;
	p32 = (u_int32type *) &tmpflt;
	chksum += *p32;
      }
    }
  ...
  if(this_node==0){
    ...
    vol3 = nx*ny*nz;
    uout = (OUTPUT_TYPE *) calloc(48*vol3, sizeof(OUTPUT_TYPE));
    if(uout == NULL) { 
	...
	if(currentnode==0){
	  ...
	  for(mu=0; mu<4; ++mu) {
		...
		uout[2*(b+3*a)+12*mu+48*j] 
		    = (OUTPUT_TYPE) s->link[mu].e[a][b].real + 0.0f;
		uout[1+2*(b+3*a)+12*mu+48*j] 
		    = (OUTPUT_TYPE) s->link[mu].e[a][b].imag + 0.0f;
	      }
	    }
	  }
	}
	  ...
	  for(mu=0; mu<4; ++mu) {
	    for(a=0; a<2; ++a) {
	      for(b=0; b<3; ++b) {
		uout[2*(b+3*a)+12*mu+48*j] 
		    = (OUTPUT_TYPE) lbuf[mu].e[a][b].real + 0.0f;
		uout[1+2*(b+3*a)+12*mu+48*j] 
		    = (OUTPUT_TYPE) lbuf[mu].e[a][b].imag + 0.0f;
	      }
	    }
	  }
	}
