...
/*									      */
/*   complex cmplx(double r, double i);           (r,i)   		      */
...
/*   complex ce_itheta(double theta);      exp(i*theta)   		      */
/*									      */
/* The following macros are provided, which work for BOTH single and double   */
/* precision and for mixtures:						      */
/*									      */
/* 1) Macros which appear to return values (double or double, as appropriate): */
...
#if defined(SPEC_CPU_NEED_COMPLEX_TYPES)
...
#define carg(a) (atan2((double)(*a).imag, (double)(*a).real ) )
								/*   b = a*   */
#define dcarg carg
#define CONJG(a,b) { (b).real = (a).real + 0.0f; (b).imag = -(a).imag + 0.0f; }
								/*  c = a + b */
