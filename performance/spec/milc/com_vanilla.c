...
int subl, nsubl, *othersubl;	/* for 32 sublattice case */
    ...
    if( inverse==WANT_INVERSE) {
        neighbor[n_gathers-2] = (int *)calloc(sites_on_node, sizeof(int));
        if(neighbor[n_gathers-2]==NULL){
            printf("NODE %d: no room for neighbor vector\n",this_node);
            terminate(1);
        }
        neighbor[n_gathers-1] = (int *)calloc(sites_on_node, sizeof(int));
    ...
    othersubl = (int *)calloc(N_SUBL32, sizeof(int));
    if(othersubl == NULL){
...
/* Broadcast double precision doubleing point number from node zero */
