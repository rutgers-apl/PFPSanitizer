...
typedef struct {
  ...
  double resid;        /* desired residual - 
			 normalized as sqrt(r*r)/sqrt(src_e*src_e */
  double size_r;       /* resulting residual */
...
typedef struct {
  double Kappa;        /* hopping */
  double Clov_c;       /* Perturbative clover coeff */
  double U0;           /* Tadpole correction to Clov_c */
...
typedef struct {
  ...
  double r0;           /* source size for w_source */
