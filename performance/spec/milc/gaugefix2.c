...
/* Prototype...
   ...
   gaugefix(TUP,(double)1.5,500,(double)1.0e-7,
   ...
   rephase( OFF );
   gaugefix(YUP,(double)1.8,500,(double)2.0e-6,
...
void gaugefixscratch(field_offset diffmat, field_offset sumvec)
{
  ...
  if(diffmat_offset < 0)
    {
      diffmatp = (su3_matrix *)calloc(sites_on_node, sizeof(su3_matrix));
      if(diffmatp == NULL)
	{
  ...
  if(sumvec_offset < 0)
    {
      sumvecp = (su3_vector *)calloc(sites_on_node, sizeof(su3_vector));
      if(sumvecp == NULL)
	{
