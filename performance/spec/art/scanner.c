...
void train_match_slice(int spot, int start, int end, int f1res){
  start_slice();
  for (j=start;j<end && !f1res ;j++)
  {
    ...
    tnorm = sqrt((double) tnorm);
    for (tj=0;tj<numf1s;tj++)
      f1_layer[tj].Q = f1_layer[tj].P + 0.0f;
...
{ 
  ...
  { 
    f1res = 0;
    train_match_slice(spot, 0, 4, f1res);
    train_match_slice(spot, 4, 8, f1res);
    train_match_slice(spot, 8, 9, f1res);
#ifdef DEBUG
    if (DB1) print_f12();
...
void match_slice(int start, int end){
  start_slice();
  int f1res = 0;
  int ti,tj,tresult;
  double tnorm;
  double xr,qr,tsum,ttemp;
  for (int j=start;j<end && !f1res ;j++)
  {
    /* Compute F1 layer - W values */
    tnorm = 0;
    for (ti=0;ti<numf1s;ti++)
...
void load_train(char *trainfile,int mode, int objects)
{
  ...
  {
    t = superbuffer[i];
    f1_layer[i].I[spot] = (double) t + 0.0f;
  }
    ...
    {  
      ...
      for (m=j;m<(lheight+j);m++)
        for (n=i;n<(lwidth+i);n++)
          f1_layer[k++].I[0] = cimage[m][n] + 0.0f;
      pass_flag =0;
      match();
      if (pass_flag==1)
      {
